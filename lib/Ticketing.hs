...
data SeatingSlice = SeatingSlice
  ...
  } deriving (Show, Generic, Csv.FromNamedRecord, NFData)
...
--   lowest cost.
findOptimalSeatingCombinations
  :: Int
  -- ^ Size of the party
  -> Float
  -- ^ Combined budget
  ...
  -- ^ Available seats
  -> [[Seat]]
findOptimalSeatingCombinations partySize budget availableSeats =
  let 
    ...
    adjacentSeatsBySection :: Map Text [[Seat]] = 
    ...
    piggybackSeatsBySection :: Map Text [[Seat]] = 
    ...
    optimalSeats :: [[Seat]] = 
      join $ M.elems adjacentSeatsBySection <> M.elems piggybackSeatsBySection
  in
    optimalSeats
  where
    ...
    findAdjacentSeats seats =
      let
        ...
        subslicesWithinCriteria = 
          filter (isCorrectSizeForParty .&&. canFitWithinBudget .&&. hasNoDiscontinuities) <$> subslicesByRows
      in
        join subslicesWithinCriteria
    ...
    findPiggybackSeats seats =
      let
        ...
        tuplesWithinCriteria = 
          filterPiggybackSeatsInRowPair <$> tuplesOfAdjacentRows
      in
        join tuplesWithinCriteria
      where
        ...
        filterPiggybackSeatsInRowPair :: ([Seat], [Seat]) -> [[Seat]]
        filterPiggybackSeatsInRowPair (row1,  row2) =
          let
          ...
          in
            filter (isCorrectSizeForParty .&&. canFitWithinBudget .&&. isPiggybackShape) subslices
